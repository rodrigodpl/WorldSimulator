...
public class WS_Nation
{
    ...
    public void Populate()
    {
        ...
        foreach (WS_Tile tile in nationTiles)
        {
            ...
            if (tile != capital)
            {
                tile.Populate(Mathf.FloorToInt(Random.Range(0.75f, 1.25f) * WS_WorldGenerator.startingPop));
                tile.urbanPercentile = Random.Range(0.05f, 0.15f);
            }
            else
            {
                tile.Populate(Mathf.FloorToInt(Random.Range(0.75f, 1.25f) * WS_WorldGenerator.startingPop * 2.0f));
