...
public enum ResourceType {  NONE = -1,
                            IRON, COPPER, TIN, SILVER, GOLD, COAL,
                            PASTURES, FISH, HUNT, FURS, SPICES, OPIOIDS,
                            GRANITE, CLAY, MARBLE, JADE, SALT, WOOD,
                            MAX
}
...
public class WS_ResourceSource
{
    ...
    public bool isMetal()
    {
        return (type == ResourceType.IRON || type == ResourceType.COPPER || type == ResourceType.TIN ||
                type == ResourceType.SILVER || type == ResourceType.GOLD || type == ResourceType.COAL);
    }

    public bool isOrganic()
    {
        return (type == ResourceType.PASTURES || type == ResourceType.FISH || type == ResourceType.SPICES ||
                type == ResourceType.HUNT || type == ResourceType.FURS || type == ResourceType.OPIOIDS);
    }

    public bool isStone()
    {
        return (type == ResourceType.GRANITE || type == ResourceType.CLAY || type == ResourceType.MARBLE ||
                type == ResourceType.JADE || type == ResourceType.SALT || type == ResourceType.WOOD);
    }
