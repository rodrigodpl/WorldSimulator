    ...
    public void Render()
    {
        ...
        switch (WS_FilterPanel.rendermode)
        {
            ...
            case WorldRenderMode.RESOURCE:
                ...
                if ((WS_FilterPanel.resFilter == ResFilter.METALS   && !tile.resource.isMetal()) ||
                   (WS_FilterPanel.resFilter == ResFilter.STONE     && !tile.resource.isStone()) ||
                   (WS_FilterPanel.resFilter == ResFilter.ORGANIC   && !tile.resource.isOrganic()))
                    renderColor = Color.white;
                else
                {
        ...
        if (WSUI_Controller.selectedTile == tile)
        {
            if (!blinkReverse)
            {
        ...
        if (!lastRenderColor.Equals(renderColor) || blinkIntensity != 0.0f)
        {
            ...
            if (blinkIntensity != 0.0f && WSUI_Controller.selectedTile != tile)
                blinkIntensity = 0.0f;
