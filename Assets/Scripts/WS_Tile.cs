...
public class WS_Tile
{
    ...
    public void addPopulation(WS_Culture migratingCulture, float ruralPop, float urbanPop)
    {
        urbanPercentile = (population * urbanPercentile + urbanPop) / (population * (1.0f - urbanPercentile) + ruralPop);
    ...
    public void updateData()
    {
        ...
        if (population < 0.0f)
        {
            nation.nationTiles.Remove(this);
            population = 0.0f;
            nation = null;
            return;
        }

        if (urbanPercentile < 0.4f) settlement = Settlement.VILLAGE;
        else if (urbanPercentile < 0.7f) settlement = Settlement.TOWN;
        else settlement = Settlement.CITY;

        foreach (WS_Tile neighbor in Neighbors())
        {
            if (neighbor.seaBody && !isCoastal) isCoastal = true;
            if (neighbor.population > 0.0f && neighbor.nation != nation && !isFrontier) isFrontier = true;
            if (neighbor.habitability + mainCulture.survivalism > 10.0f && neighbor.population == 0.0f && !neighbor.seaBody && !isBorder) isBorder = true;
        }
        ...
        cultureStrength = cultureStrength * strengthMultiplier;
    }
